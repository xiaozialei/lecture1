...
class Astar:
     ...
     def search(self,map:Map,start_pos=(0,0),end_pos=(1,1)):
          ...
          index = 0
          priority = self.calcDiag(start_pos,end_pos)
          map.explored.append(start_pos)
          heapq.heappush(self.pq,(priority,index,start_pos))
          while self.pq:
               ...
               for i in range(-1,2,1):
                    for j in range(-1,2,1):
                         ...
                         if nei_pos not in self.tb or dis<self.tb[nei_pos].dis:
                              ...
                              priority = self.calcDiag(nei_pos,end_pos)
                              index=index+1
                              map.explored.append(nei_pos)
                              heapq.heappush(self.pq,(priority,index,nei_pos))
          return 
     ...
     def calcDiag(self,start,end):
          ...
          if x_offset>=y_offset:
               heu_dis = 1.414*y_offset+x_offset-y_offset
          else:
               heu_dis = 1.414*x_offset+y_offset-x_offset
          return heu_dis+self.tb[start].dis
...
if __name__=='__main__':
    ...
    start_pos = (1,1)
    end_pos = (6,1)
    astar.search(map,start_pos,end_pos)
    astar.printPath(map,end_pos)
    print('*************************')
    map.showMap()
    print('*************************')
    map.printExplored()
